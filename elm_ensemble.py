...
class ELMEnsemble(BaseEstimator, RegressorMixin):
    ...
    def model_variance(self, heterosk=False, approx=True):
        '''
        ...
        if heterosk == False :
            ...
            if approx == True :
                ...
                var1 = np.einsum('nim, ijm, kjl, nkl -> n', H_predict, H_pinvs, 
                                 H_pinvs, H_predict, optimize = 'greedy')
...
class ELMEnsembleRidge(BaseEstimator, RegressorMixin):
...
class ELMEnsembleRidgeCV(BaseEstimator, RegressorMixin):
